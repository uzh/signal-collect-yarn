 ...
 */
object DeployableEfficientPageRankLoader extends YarnDeployableAlgorithm {
  def execute(parameters: Map[String, String], nodeActors: Array[ActorRef]) {
    val g = new GraphBuilder[Int, Double].withShutdownActorSystem(false).withMessageBusFactory(new BulkAkkaMessageBusFactory(1024, false)).build
