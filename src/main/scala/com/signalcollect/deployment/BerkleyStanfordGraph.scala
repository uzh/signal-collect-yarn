...
class BerkleyStanfordGraph extends DeployableAlgorithm {
  override def execute(parameters: Map[String, String], graphBuilder: GraphBuilder[Any, Any]) {
    ...
    for( id  <- 0 until numberOfNodes){
      ...
      graph.loadGraph(getFileLoader(length, numberOfNodes, id, url), Some(id*4))
    }
...
case class BerkleyStanleyLoader(fileUrl: String, start: Long, end: Long) extends Iterator[GraphEditor[Int, Double] => Unit] {
  lazy val in = getBufferedReader
  var cnt = start
  var edgeCnt = 0
  val startTime = System.currentTimeMillis
  
  def addEdge(verticeIds: (Int,Int))(graphEditor: GraphEditor[Int, Double]) {
    //    graphEditor.addVertex(vertices._1)
    //    graphEditor.addVertex(vertices._2)
    graphEditor.addEdge(verticeIds._1, new PlaceholderEdge[Int](verticeIds._2).asInstanceOf[Edge[Int]])
  }
